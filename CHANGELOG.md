...
- Fixed an out-of-bounds read in whisper.cpp reachable from safe code: `VadSegments::get_segment_t0()` / `get_segment_t1()` passed the segment index to whisper.cpp, which does not bounds-check it. They now panic on an out-of-range index, like slice indexing and the `WhisperState` result getters.
